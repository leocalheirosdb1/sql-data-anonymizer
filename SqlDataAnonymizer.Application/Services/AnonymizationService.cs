...
public sealed class AnonymizationService : IAnonymizationService
{
    ...
    public async Task<Guid> StartAnonymizationAsync(string server, string database, DatabaseType dbType)
    {
        ...
        _logger.LogInformation("Anonimização iniciada - JobId: {JobId}, Server: {Server}, Database: {Database}, Type: {Type}", 
    ...
    private async Task ProcessJobAsync(AnonymizationJobDto job)
    {
        try
        {
